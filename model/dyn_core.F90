...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_map, n_split, zvir, cp, akap, cappa, grav, hydrostatic,  &
                     ...
                     init_step, i_pack, end_step, heat_source, diss_est, smag_q, consv, te0_2d, time_total)
    ...
    real, intent(inout) :: smag_q(bd%isd:,bd%jsd:,1:) ! diffusion coefficient (nu*dt = dimensionless)
...
!                    call init_ijk_mem(isd,ied, jsd,jed, npz, vt, 0.)
           if (flagstruct%smag2d > 1.e-5) then
              ...
              allocate(dudz(isd:isd,jsd:jed,npz))
              allocate(dvdz(isd:isd,jsd:jed,npz))
           endif
...
! Empty the "flux capacitors"
    ...
    if ( flagstruct%convert_ke .or. flagstruct%vtdm4> 1.E-4 .or. flagstruct%smag2d > 1.e-4 ) then
         n_con = npz
    else
...
!$OMP                                  heat_source,diss_est,radius,dudz,dvdz,smag_q)              &
...
! Correction factor for 3D Coriolis force
       ...
       call d_sw(vt(isd,jsd,k), delp(isd,jsd,k), ptc(isd,jsd,k),  pt(isd,jsd,k),      &
...
#endif
                  kgb, heat_s, diss_e, zvir, sphum, nq,  q,  k,  npz, &
                  flagstruct%inline_q,  dt,  &
                  ...
                  dudz(isd:,jsd:,k), dvdz(isd:,jsd:,k), smag_q(isd:,jsd:,k), &
...
!$OMP parallel do default(none) shared(npy,jed,npz,istart,iend,delzBC,gz,a1,a2)
 ...
 subroutine compute_dudz(bd, npz, u, v, dudz, dvdz, gz, dp_ref)
   ...
   do j=jsd,jed
      ...
      call edge_profile1(v(isd:ied+1,j,:), ve, isd,  ied+1, npz, dp_ref, 1)
      do k=1,npz
         do i=isd+1,ied
   ...
   do j=jsd+1,jed
      call edge_profile1(u(isd:ied,j,:), ue, isd, ied, npz, dp_ref, 1)
      do k=1,npz
         do i=isd,ied
