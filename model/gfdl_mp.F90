...
module gfdl_mp_mod
    ...
    real :: dlin = 0.25 ! "d" in Lin et al. (1983)
    real :: elin = 1.0 ! "e" in Lin et al. (1983), similar to a, but for graupel
    real :: flin = 0.5 ! "f" in Lin et al. (1983), similar to b, but for graupel
    ...
    namelist / gfdl_mp_nml / &
        ...
        const_vr, rthresh, ccn_l, ccn_o, igflag, c_paut, tau_imlt, &
        tau_v2l, tau_l2v, tau_i2s, tau_l2r, qi_lim, ql_gen, do_hail, inflag, &
        c_psacw, c_psaci, c_pracs, c_psacr, c_pgacr, c_pgacs, c_pgacw, c_pgaci, &
        z_slope_liq, z_slope_ice, prog_ccn, c_pracw, c_praci, alin, blin, clin, &
        dlin, elin, flin, rad_snow, rad_graupel, rad_rain, cld_min, &
...
subroutine setup_mp
    ...
    vcong = elin * gamma (4 + flin) / 6.0 * gcon
    vconh = elin * vcong * sqrt (rhoh / rhog)
    ...
    csacr = pisq * rnzs * rnzr * rhor
    if (do_hail) then
        cgacr = pisq * rnzh * rnzr * rhor
        cgacs = pisq * rnzh * rnzs * rhos
    else
        cgacr = pisq * rnzg * rnzr * rhor
    ...
    do i = 1, 3
        do k = 1, 4
            acco (i, k) = acc (i) / &
                (exp ((7 - i) * 0.25 * log (act (2 * k - 1))) * exp (i * 0.25 * log (act (2 * k))))
        enddo
    enddo
    ...
    cracw = pi * rnzr * alin * gamma (3 + blin) / (4. * exp (0.25 * (3 + blin) * log (act (2))))
    craci = pi * rnzr * alin * gamma (3 + blin) / (4. * exp (0.25 * (3 + blin) * log (act (2))))
    csacw = pi * rnzs * clin * gamma (3 + dlin) / (4. * exp (0.25 * (3 + dlin) * log (act (1))))
    csaci = pi * rnzs * clin * gamma (3 + dlin) / (4. * exp (0.25 * (3 + dlin) * log (act (1))))
    if (do_hail) then
        cgacw = pi * rnzh * elin * gamma (3 + flin) * gcon / (4. * exp (0.25 * (3 + flin) * log (act (6))))
        cgaci = pi * rnzh * elin * gamma (3 + flin) * gcon / (4. * exp (0.25 * (3 + flin) * log (act (6))))
    else
        cgacw = pi * rnzg * elin * gamma (3 + flin) * gcon / (4. * exp (0.25 * (3 + flin) * log (act (6))))
        cgaci = pi * rnzg * elin * gamma (3 + flin) * gcon / (4. * exp (0.25 * (3 + flin) * log (act (6))))
    endif
    ...
    ! -----------------------------------------------------------------------
    ! rain evaporation, snow sublimation, and graupel or hail sublimation
    ! -----------------------------------------------------------------------
    
    crevp (1) = 2. * pi * vdifu * tcond * rvgas * rnzr
    crevp (2) = 0.78 / sqrt (act (2))
    crevp (3) = 0.31 * scm3 * gamma ((5 + blin) / 2) * sqrt (alin / visk) / &
        exp (0.25 * (5 + blin) / 2 * log (act (2)))
    crevp (4) = tcond * rvgas
    crevp (5) = hlv ** 2 * vdifu
    
    cssub (1) = 2. * pi * vdifu * tcond * rvgas * rnzs
    cssub (2) = 0.78 / sqrt (act (1))
    cssub (3) = 0.31 * scm3 * gamma ((5 + dlin) / 2) * sqrt (clin / visk) / &
        exp (0.25 * (5 + dlin) / 2 * log (act (1)))
    cssub (4) = tcond * rvgas
    ...
    cgsub (3) = 0.31 * scm3 * gamma ((5 + flin) / 2) * sqrt (elin * gcon / visk) / &
        exp (0.25 * (5 + flin) / 2 * log (act (6)))
    cgsub (4) = tcond * rvgas
    cgsub (5) = (hlv + hlf) ** 2 * vdifu

    ! -----------------------------------------------------------------------
    ! rain freezing
    ! -----------------------------------------------------------------------
    
    cgfr (1) = 20.e2 * pisq * rnzr * rhor / exp (1.75 * log (act (2)))
    cgfr (2) = 0.66
...
subroutine mpdrv (hydrostatic, ua, va, wa, delp, pt, qv, ql, qr, qi, qs, &
    ...
    real (kind = r_grid), dimension (ks:ke) :: tz, te1, te2
    ...
    do i = is, ie
        ...
        do n = 1, ntimes
            ...
            call sedimentation (dts, ks, ke, tz, qvz, qlz, qrz, qiz, qsz, qgz, &
                dz, dp, vtr, vti, vts, vtg, r1, i1, s1, g1, u, v, w, den, denfac, dte (i))
            ...
            call warm_rain (dts, ks, ke, dp, dz, tz, qvz, qlz, qrz, qiz, qsz, qgz, &
            ...
            call ice_cloud (ks, ke, tz, qvz, qlz, qrz, qiz, qsz, qgz, den, &
            ...
            call subgrid_z_proc (ks, ke, den, denfac, dts, rh_adj, tz, qvz, qlz, &
                qrz, qiz, qsz, qgz, qaz, dp, ccn, cin, cond, dep, reevap, sub)
    ...
    if (consv_checker) then
        if (abs (sum (te_end) + sum (te_b_end) - sum (te_beg) - sum (te_b_beg)) / &
            ...
            print *, "gfdl_mp te: ", (sum (te_beg) + sum (te_b_beg)) / sum (gsize ** 2), &
        ...
        if (abs (sum (tw_end) + sum (tw_b_end) - sum (tw_beg) - sum (tw_b_beg)) / &
            ...
            print *, "gfdl_mp tw: ", (sum (tw_beg) + sum (tw_b_beg)) / sum (gsize ** 2), &
        ...
        if (tz (k) .gt. t_wfr .and. qr (k) .gt. qcmin .and. dqv .gt. 0.0 .and. qsat .gt. q_minus) then
            ...
            if (use_rhc_revap) then
                ...
                if (rh_tem .lt. rhc_revap) then
                    sink = psub (t2, dq, qden, qsat, crevp, den (k), denfac (k), blin)
...
subroutine pracw (ks, ke, dts, tz, qv, ql, qr, qi, qs, qg, den, denfac)
    ...
    do k = ks, ke
        if (tz (k) .gt. t_wfr .and. qr (k) .gt. qcmin .and. ql (k) .gt. qcmin) then
            qden = qr (k) * den (k)
            sink = dts * acr2d (qden, cracw, denfac (k), blin)
    ...
    if (irain_f .eq. 0) then
        ...
        do k = ks, ke
            if (tz (k) .gt. t_wfr .and. ql (k) .gt. qcmin) then
                ...
                dq = 0.5 * (ql (k) + dl (k) - qc)
                if (dq .gt. 0.) then
                    c_praut (k) = cpaut * exp (- 1. / 3. * log (ccn (k) * rhor))
                    sink = min (1., dq / dl (k)) * dts * c_praut (k) * den (k) * exp (so3 * log (ql (k)))
                    sink = min (ql (k), sink)
                    call update_qq (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                        0., - sink, sink, 0., 0., 0.)
                endif
...
subroutine ice_cloud (ks, ke, tz, qv, ql, qr, qi, qs, qg, den, &
    ...
    if (.not. do_warm_rain_mp) then
        ...
        call pimlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, lcpk, icpk, tcpk, tcp3)
        ...
        call psmlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        ...
        call pgmlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        ...
        ! -----------------------------------------------------------------------
        ! snow accretion and graupel freezing
        ! -----------------------------------------------------------------------
                
        call psacr_pgfr (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
            vtr, vts, lcpk, icpk, tcpk, tcp3)
        ...
        call pgacw_pgacr (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
            vtr, vtg, lcpk, icpk, tcpk, tcp3)
...
subroutine pimlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, lcpk, icpk, tcpk, tcp3)
    ...
    do k = ks, ke
        if (tz (k) .gt. tice_mlt .and. qi (k) .gt. qcmin) then
            ...
            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., tmp, sink - tmp, - sink, 0., 0., te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
        endif
    enddo
...
subroutine pifr (ks, ke, qv, ql, qr, qi, qs, qg, tz, te8, den, lcpk, icpk, tcpk, tcp3)
    ...
    real :: tc, factor, tmp, sink, qim

    do k = ks, ke
        if (tz (k) .lt. t_wfr .and. ql (k) .gt. qcmin) then
            tc = t_wfr - tz (k)
            ...
            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., - sink, 0., tmp, sink - tmp, 0., te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
        endif
    enddo
...
subroutine psmlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        vtr, vts, lcpk, icpk, tcpk, tcp3)
    ...
    do k = ks, ke
        ...
        if (tc .ge. 0. .and. qs (k) .gt. qcmin) then
            ...
            if (ql (k) .gt. qcmin) then
                factor = acr2d (qden, csacw, denfac (k), dlin)
            ...
            sink = max (0., pmlt (tc, dq, qden, psacw, psacr, csmlt, den (k), denfac (k), dlin))

            sink = min (qs (k), (sink + pracs) * dts, tc / icpk (k))
            tmp = min (sink, dim (qs_mlt, ql (k)))
            
            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., tmp, sink - tmp, 0., - sink, 0., te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
...
subroutine pgmlt (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        vtr, vtg, lcpk, icpk, tcpk, tcp3)
    ...
    do k = ks, ke
        ...
        if (tc .ge. 0. .and. qg (k) .gt. qcmin) then
            ...
            qden = qg (k) * den (k)
            if (ql (k) .gt. qcmin) then
                factor = acr2d (qden, cgacw, denfac (k), flin)
            ...
            tin = tz (k)
            dq = iqs (tin, den (k), dqdt) - qv (k)
            sink = max (0., pmlt (tc, dq, qden, pgacw, pgacr, cgmlt, den (k), denfac (k), flin))

            sink = min (qg (k), sink * dts, tc / icpk (k))

            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., 0., sink, 0., 0., - sink, te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
    ...
    do k = ks, ke
        ...
        if (tc .lt. 0. .and. qi (k) .gt. qcmin) then
            ...
            if (qs (k) .gt. qcmin) then
                factor = dts * acr2d (qden, csaci, denfac (k), dlin)
    ...
    do k = ks, ke
        ...
        if (tc .lt. 0. .and. qi (k) .gt. qcmin) then
            ...
            if (qg (k) .gt. qcmin) then
                factor = dts * acr2d (qden, cgaci, denfac (k), flin)
                sink = factor / (1. + factor) * qi (k)
            endif
...
subroutine psacr_pgfr (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        vtr, vts, lcpk, icpk, tcpk, tcp3)
    ...
    do k = ks, ke
        ...
        if (tc .lt. 0. .and. qr (k) .gt. qcmin) then
            ...
            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., 0., - sink, 0., psacr, pgfr, te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
...
subroutine pgacs (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, den, vts, vtg)
    ...
    do k = ks, ke
        ...
        if (tc .lt. 0. .and. qs (k) .gt. qcmin) then
            ...
            sink = 0
            if (qg (k) .gt. qcmin) then
                sink = dts * acr3d (vtg (k), vts (k), qs (k), qg (k), cgacs, acco (1, 4), den (k))
            endif
...
subroutine pgacw_pgacr (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, den, denfac, &
        vtr, vtg, lcpk, icpk, tcpk, tcp3)
    ...
    do k = ks, ke
        ...
        if (tc .lt. 0. .and. qg (k) .gt. qcmin) then
            ...
            if (ql (k) .gt. qcmin) then
                qden = qg (k) * den (k)
                factor = dts * acr2d (qden, cgacw, denfac (k), flin)
            ...
            call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                0., - pgacw, - pgacr, 0., 0., sink, te8 (k), tz (k), &
                lcpk (k), icpk (k), tcpk (k), tcp3 (k))
...
subroutine subgrid_z_proc (ks, ke, den, denfac, dts, rh_adj, tz, qv, ql, qr, &
        qi, qs, qg, qa, dp, ccn, cin, cond, dep, reevap, sub)
    ...
    if (.not. do_warm_rain_mp) then
        ...
        call pinst (ks, ke, qv, ql, qr, qi, qs, qg, qa, tz, dp, te8, den, &
    ...
    call pcond_pevap (ks, ke, dts, qv, ql, qr, qi, qs, qg, qa, tz, dp, te8, den, &
    ...
    if (.not. do_warm_rain_mp) then
        ...
        call pcomp (ks, ke, qv, ql, qr, qi, qs, qg, tz, te8, lcpk, icpk, tcpk, tcp3)
 
        ! -----------------------------------------------------------------------
        ! bigg freezing mechanism
        ! -----------------------------------------------------------------------
                
        call pbigg (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, ccn, lcpk, icpk, tcpk, tcp3)

        ! -----------------------------------------------------------------------
        ! cloud ice deposition and sublimation
        ! -----------------------------------------------------------------------
            
        call pidep_pisub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
        ...
        call psdep_pssub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
        ...
        call pgdep_pgsub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
            denfac, lcpk, icpk, tcpk, tcp3, dep, sub)
...
subroutine pinst (ks, ke, qv, ql, qr, qi, qs, qg, qa, tz, dp, te8, den, &
        lcpk, icpk, tcpk, tcp3, rh_adj, dep, sub, reevap)
    ...
    do k = ks, ke
        ...
        if (tin .gt. t_sub + 6.) then
            ...
            if (rh .lt. rh_adj) then
                ...
                sub = sub + qi (k) * dp (k)
                call update_qt (qv (k), ql (k), qr (k), qi (k), qs (k), qg (k), &
                    ql (k) + qi (k), - ql (k), 0., - qi (k), 0., 0., te8 (k), tz (k), &
                    lcpk (k), icpk (k), tcpk (k), tcp3 (k))
                cycle
            endif
        endif
...
subroutine pcond_pevap (ks, ke, dts, qv, ql, qr, qi, qs, qg, qa, tz, dp, te8, den, &
        lcpk, icpk, tcpk, tcp3, cond, reevap)
...
subroutine pcomp (ks, ke, qv, ql, qr, qi, qs, qg, tz, te8, lcpk, icpk, tcpk, tcp3)
...
subroutine pbigg (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, te8, ccn, lcpk, icpk, tcpk, tcp3)
...
subroutine pidep_pisub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
        lcpk, icpk, tcpk, tcp3, cin, dep, sub)
...
subroutine psdep_pssub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
        denfac, lcpk, icpk, tcpk, tcp3, dep, sub)
    ...
    do k = ks, ke
        ...
        if (qs (k) .gt. qcmin) then
            ...
            t2 = tz (k) * tz (k)
            dq = qsi - qv (k)
            pssub = psub (t2, dq, qden, qsi, cssub, den (k), denfac (k), dlin)
            pssub = dts * pssub
            dq = dq / (1. + tcpk (k) * dqdt)
            if (pssub .gt. 0.) then
...
subroutine pgdep_pgsub (ks, ke, dts, qv, ql, qr, qi, qs, qg, tz, dp, te8, den, &
        denfac, lcpk, icpk, tcpk, tcp3, dep, sub)
    ...
    do k = ks, ke
        ...
        if (qg (k) .gt. qcmin) then
            ...
            t2 = tz (k) * tz (k)
            dq = qsi - qv (k)
            pgsub = psub (t2, dq, qden, qsi, cgsub, den (k), denfac (k), flin)
            pgsub = dts * pgsub
            dq = dq / (1. + tcpk (k) * dqdt)
            if (pgsub .gt. 0.) then
    ...
    do k = ks, ke
        ...
        if (cfflag .eq. 1) then
            if (rh .gt. rh_thres .and. qpz .gt. qcmin) then
                ...
                dq = h_var * qpz
                if (do_cld_adj) then
                    q_plus = qpz + dq * f_dq_p * min(1.0, max(0.0, (pz (k) - 200.e2) / (1000.e2 - 200.e2)))
                else
        ...
        if (cfflag .eq. 3) then
            if (q_cond (k) .gt. qcmin) then
                qa (k) = 1. / 50. * (5.77 * (100. - gsize / 1000.) * exp (1.07 * log (max (qcmin * 1000., q_cond (k) * 1000.))) + &
                    4.82 * (gsize / 1000. - 50.) * exp (0.94 * log (max (qcmin * 1000., q_cond (k) * 1000.))))
...
subroutine fast_sat_adj (mdt, is, ie, js, je, ng, hydrostatic, consv_te, &
    ...
    real :: d0_vap, lv00, li00, qsw, rh, ccn0, rqi, q_plus, q_minus
    real :: tc, qsi, dqdt, dq, dq0, pidep, qi_gen, qi_crt, tmp
    real :: tin, sdt, factor, qim, c_air, c_vap, dw
    real :: fac_smlt, fac_r2g, fac_i2s, fac_imlt, fac_l2r, fac_v2l, fac_l2v
    
    real, dimension (is:ie) :: wqsat, qpz, t0, qstar, mc_air, lhl, lhi, ccn, cin
    real, dimension (is:ie) :: den, q_liq, q_sol, q_cond, src, sink, hvar, icp2, lcp2, tcp2, tcp3
    
    real (kind = r_grid), dimension (is:ie, js:je) :: te_beg, te_end, tw_beg, tw_end
    
    real (kind = r_grid), dimension (is:ie) :: pt1, cvm
    ...
    do j = js, je
        ...
        do i = is, ie
            tin = pt1 (i)
            wqsat (i) = wqs (tin, den (i), dqdt)
            dq0 = (qv (i, j) - wqsat (i)) / (1. + tcp3 (i) * dqdt)
            if (dq0 .gt. 0.) then
                src (i) = min (dq0, max (ql_gen - ql (i, j), fac_v2l * dq0))
            else
                ! sjl, 20170703
        ...
        do i = is, ie
            tc = tice - pt1 (i)
            if (ql (i, j) .gt. qcmin .and. tc .gt. 0.) then
                sink (i) = 100. / (rhow * ccn (i)) * mdt * (exp (0.66 * tc) - 1.) * ql (i, j) ** 2
        ...
        do i = is, ie
            ...
            elseif (pt1 (i) .lt. tice) then
                ...
                if (qi (i, j) .gt. qcmin) then
                    ...
                    pidep = mdt * dq * 4.0 * 11.9 * exp (0.5 * log (qi (i, j) * den (i) * cin (i))) &
...
subroutine cld_eff_rad (is, ie, ks, ke, lsm, p, delp, t, qw, qi, qr, qs, qg, &
    ...
    real :: dpg, rho, ccnw, mask, cor, tc0, bw
    ...
    do k = ks, ke
        ...
        do i = is, ie
            ...
            if (reiflag .eq. 5) then
                ...
                if (qmi (i, k) .gt. qcmin) then
                    qci (i, k) = dpg * qmi (i, k) * 1.0e3
                    bw = - 2. + 1.e-3 * log10 (rho * qmi (i, k) / 50.e-3) * exp (1.5 * log (max (1.e-10, - tc0)))
            ...
            if (qmr (i, k) .gt. qcmin) then
                ...
                rer (i, k) = 0.5 * exp (log (gamma (4 + blin) / 6) / 0.80) / lambdar * 1.0e6
            ...
            if (qms (i, k) .gt. qcmin) then
                ...
                res (i, k) = 0.5 * exp (log (gamma (4 + dlin) / 6) / 0.25) / lambdas * 1.0e6
            ...
            if (qmg (i, k) .gt. qcmin) then
                ...
                reg (i, k) = 0.5 * exp (log (gamma (4 + flin) / 6) / 0.50) / lambdag * 1.0e6
...
subroutine rad_ref (is, ie, js, je, isd, ied, jsd, jed, q, pt, delp, peln, &
    ...
    real (kind = r_grid), parameter :: factor_s = gamma_seven * 1.e18 * exp (1.75 * log (1. / (pi * rhos))) &
         * (rhos / rhor) ** 2 * alpha
...
subroutine update_qt (qv, ql, qr, qi, qs, qg, dqv, dql, dqr, dqi, dqs, dqg, te8, &
    ...
    real (kind = r_grid), intent (out) :: tk
...
function acr2d (qden, c, denfac, lin)
    ...
    real, intent (in) :: qden, c, denfac, lin
    
    acr2d = denfac * c * exp (0.25 * (3 + lin) * log (qden))
...
function acr3d (v1, v2, q1, q2, c, cac, rho)
...
function vent_coeff (qden, c1, c2, denfac, lin)
    ...
    real, intent (in) :: qden, c1, c2, denfac, lin
    
    vent_coeff = c1 * sqrt (qden) + c1 * exp (0.25 * (5 + lin) / 2 * log (qden)) * sqrt (denfac)
...
function psub (t2, dq, qden, qsat, c, den, denfac, lin)
    ...
    real, intent (in) :: t2, dq, qden, qsat, c (5), den, denfac, lin
    
    psub = c (1) * t2 * dq * vent_coeff (qden, c (2), c (3), denfac, lin) / &
        (c (4) * t2 + c (5) * qsat * den)
...
function pmlt (tc, dq, qden, pxacw, pxacr, c, den, denfac, lin)
    ...
    real, intent (in) :: tc, dq, qden, pxacw, pxacr, c (5), den, denfac, lin
    
    pmlt = (c (1) * tc / den - c (2) * dq) * vent_coeff (qden, c (3), c (4), denfac, lin) + &
        c (5) * tc * (pxacw + pxacr)
...
! used for ice microphysics or warm rain microphysics
! =======================================================================
...
subroutine mqs3d (im, km, ks, tk, pa, qv, qs, dqdt)
    ...
    real, intent (in), dimension (im, km) :: tk, pa, qv
