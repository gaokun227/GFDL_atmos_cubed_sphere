...
!-------------------
       if (Atm(n)%flagstruct%write_3d_diags) then
          ...
          id_epv = register_diag_field ( trim(field), 'epv', axes(1:3), Time,       &
               'Ertel''s potential vorticity', 'PVU', missing_value=missing_value )
...
!               wk(i,j,k) =  (Atm(n)%pt(i,j,k)-300.)/Atm(n)%pkz(i,j,k) * pk0
       ...
       if (id_epv > 0 .and. .not. Atm(n)%flagstruct%hydrostatic) then
          if (allocated(a3)) deallocate(a3)
          allocate(a3(isc:iec,jsc:jec,npz))
          do k=1,npz
             a1d(k) = Atm(n)%ak(k+1)-Atm(n)%ak(k) + (Atm(n)%bk(k+1)-Atm(n)%bk(k))*1.E5
          enddo
          call ertel_pv(Atm(n)%u, Atm(n)%v, Atm(n)%w, Atm(n)%pt, Atm(n)%delp, Atm(n)%delz, Atm(n)%pkz, a3, a1d ,Atm(n)%gridstruct, Atm(n)%domain, Atm(n)%bd, Atm(n)%npx, Atm(n)%npy, npz)
          used = send_data (id_epv, a3, Time)
          deallocate(a3)
       endif
...
601       format(3x, A16, ' Valid ', I4, I2.2, I2.2, '.', I2.2, 'Z  Init ', I4, I2.2, I2.2, '.', I2.2, 'Z')
  ...
  !Still needs some bugs sorted out
  subroutine ertel_pv(u, v, w, pt, delp, delz, pkz, epv, dp0, gridstruct, domain, bd, npx, npy, npz)

    use nh_utils_mod, only:edge_scalar
    
    type(fv_grid_bounds_type), intent(IN) :: bd
    integer, intent(IN) :: npx, npy, npz
    real, intent(IN)  :: u(bd%isd:bd%ied,bd%jsd:bd%jed+1,npz)
    real, intent(IN)  :: v(bd%isd:bd%ied+1,bd%jsd:bd%jed,npz)
    real, intent(INOUT)  :: w(bd%isd:bd%ied,bd%jsd:bd%jed,npz) !a2b demands an inout directive
    real, intent(IN)  :: pt(bd%isd:bd%ied,bd%jsd:bd%jed,npz)
    ...
    real, intent(IN)  :: dp0(npz)
    real, intent(OUT)  ::  epv(bd%is:bd%ie,bd%js:bd%je,npz)
    type(fv_grid_type), intent(IN) :: gridstruct
    type(domain2d), intent(INOUT) :: domain
    

    real :: ue(bd%isd:bd%ied,bd%jsd:bd%jed+1,npz+1), ve(bd%isd:bd%ied+1,bd%jsd:bd%jed,npz+1), wb(bd%isd:bd%ied,bd%jsd:bd%jed,npz), delz_h(bd%isd:bd%ied,bd%jsd:bd%jed,npz), dzb(bd%isd:bd%ied,bd%jsd:bd%jed,npz), xi(bd%is:bd%ie,bd%js:bd%je+1,npz), eta(bd%is:bd%ie+1,bd%js:bd%je,npz), vortz(bd%is:bd%ie,bd%js:bd%je,npz+1), theta(bd%isd:bd%ied,bd%jsd:bd%jed,npz)

    integer :: i, j, k
    real :: th1, th2, rrho, rvol
    real, PARAMETER :: p0 = 1000.e2
    real, PARAMETER :: rpk = exp(kappa*log(p0))
    integer :: is,  ie,  js,  je
    integer :: isd, ied, jsd, jed
    integer :: ng
    ...
    !1. Compute top/bottom edge horizontal velocities (need to update routines for non-uniform grid)
    !   Use dp0 (reference height) to compute edge values
    !   Use ppme to compute this?? requires full delp and thus a halo update
    !    Want to multiply in dx and dy as appropriate
    !do j=js,je+1
    !   call edge_scalar(u(is,j,1), ue(is,j,1), bd%is, bd%ie, npz, 1)
    !enddo
    k=1
       do j=js,je+1
          do i=is,ie
             ue(i,j,k) = u(i,j,k) !* gridstruct%dy(i,j)
          enddo
       enddo
    do k=2,npz
       do j=js,je+1
          do i=is,ie
             ue(i,j,k) = 0.5*(u(i,j,k-1)+u(i,j,k)) !* gridstruct%dy(i,j)
          enddo
       enddo
    enddo
    k=npz+1
       do j=js,je+1
          do i=is,ie
             ue(i,j,k) = u(i,j,npz) !* gridstruct%dy(i,j)
          enddo
       enddo
    !do j=js,je
    !   call edge_scalar(v(is,j,1), ve(is,j,1), bd%is, bd%ie+1, npz, 1)
    !enddo
    k=1
       do j=js,je
          do i=is,ie+1
             ve(i,j,k) = v(i,j,k) !* gridstruct%dx(i,j)
          enddo
       enddo
    do k=2,npz
       do j=js,je
          do i=is,ie+1
             ve(i,j,k) = 0.5*(v(i,j,k-1)+v(i,j,k)) !* gridstruct%dx(i,j)
          enddo
       enddo
    enddo
    k=npz+1
       do j=js,je
          do i=is,ie+1
             ve(i,j,k) = v(i,j,npz) !* gridstruct%dx(i,j)
          enddo
       enddo
    

    !2. Compute corner vertical velocities and edge distances
    do k=1,npz
       call a2b_ord4(w(isd,ied,k), wb(isd,ied,k), gridstruct, npx, npy, is, ie, js, je, ng)
    enddo
    do k=1,npz
       do j=js,je
          do i=is,ie
             delz_h(i,j,k) = delz(i,j,k)
          enddo
       enddo
    enddo
    call mpp_update_domains(delz_h, domain)
    do k=1,npz
       call a2b_ord4(delz_h(isd,ied,k), dzb(isd,ied,k), gridstruct, npx, npy, is, ie, js, je, ng)
       do j=js,je+1
          do i=is,ie+1
             wb(i,j,k) = wb(i,j,k)!*dzb(i,j,k)
          enddo
       enddo
    enddo
    
    !3. Compute face-mean normal vorticity components using Stokes's theorem
    do k=1,npz
       do j=js,je
          do i=is,ie+1
             !rarea_side = 1./( 0.5*gridstruct%dy(i,j)*(dzb(i,j,k)+dzb(i,j+1,k)) ) ! unnecessary because we multiply by the area again later anyway??
             xi(i,j,k) = ( ue(i,j,k) + wb(i,j+1,k) - ue(i,j,k+1) - wb(i,j,k))
          enddo
       enddo
       do j=js,je+1
          do i=is,ie
             !rarea_side = 1./( 0.5*gridstruct%dx(i,j)*(dzb(i,j,k)+dzb(i+1,j,k)) )
             eta(i,j,k) = ( ve(i,j,k) + wb(i+1,j,k) - ve(i,j,k+1) - wb(i,j,k))
          enddo
       enddo
    enddo
    do k=1,npz+1
       do j=js,je
          do i=is,ie
             vortz(i,j,k) = (ue(i,j,k) + ve(i+1,j,k) - ue(i,j+1,k) - ve(i,j,k))
             vortz(i,j,k) = vortz(i,j,k) + gridstruct%area(i,j)*gridstruct%f0(i,j) ! absolute vertical vorticity; assuming shallow atmosphere
          enddo
       enddo
    enddo
    
    
    !4. Compute theta on all faces ( requires a more accurate calculation for top and bottom edges)
    ! Compute using dry kappa (for now) since this is what is usually expected?
    do k=1,npz
       do j=js,je
          do i=is,ie
             theta(i,j,k) = pt(i,j,k)/pkz(i,j,k)*rpk
          enddo
       enddo
    enddo
    call mpp_update_domains(theta, domain, whalo=1, ehalo=1, shalo=1, nhalo=1)
    
    !5. Sum 
    k=1
        do j=js,je
          do i=is,ie
!!$             th1 = 0.5*(theta(i-1,j,k) + theta(i,j,k))
!!$             th2 = 0.5*(theta(i,j,k) + theta(i+1,j,k))
!!$             epv(i,j,k) = xi(i+1,j,k)*th2 - xi(i,j,k)*th1
!!$
!!$             th1 = 0.5*(theta(i,j-1,k) + theta(i,j,k))
!!$             th2 = 0.5*(theta(i,j,k) + theta(i,j-1,k))
!!$             epv(i,j,k) = epv(i,j,k) + eta(i,j+1,k)*th2 - eta(i,j,k)*th1

             th1 = theta(i,j,k)
             th2 = 0.5*(theta(i,j,k) + theta(i,j,k+1))
             epv(i,j,k) = vortz(i,j,k+1)*th2 - vortz(i,j,k)*th1
!             epv(i,j,k) = epv(i,j,k) + vortz(i,j,k+1)*th2 - vortz(i,j,k)*th1
          enddo
       enddo
    do k=2,npz-1
       do j=js,je
...
!!$             epv(i,j,k) = epv(i,j,k) + eta(i,j+1,k)*th2 - eta(i,j,k)*th1
             ...
             th1 = 0.5*(theta(i,j,k-1) + theta(i,j,k))
             th2 = 0.5*(theta(i,j,k) + theta(i,j,k+1))
             epv(i,j,k) = vortz(i,j,k+1)*th2 - vortz(i,j,k)*th1
!             epv(i,j,k) = epv(i,j,k) + vortz(i,j,k+1)*th2 - vortz(i,j,k)*th1
          enddo
...
!             epv(i,j,k) = epv(i,j,k) + vortz(i,j,k+1)*th2 - vortz(i,j,k)*th1
   ...
   do k=1,npz
      do j=js,je
         do i=is,ie
            rrho = grav/delp(i,j,k)
            rvol = gridstruct%rarea(i,j) !delz drops out
            epv(i,j,k) = epv(i,j,k) * rrho * rvol * 1.e6 ! PVU
         enddo
      enddo
   enddo
...
!!!DEBUG CODE!!!
   if (is_master()) then
      i = 1; j = 1 ; k = npz/2
      print*, 'u: ', ue(i,j,k), u(i,j,k), ue(i,j,k+1)
      print*, 'v: ', ve(i,j,k), v(i,j,k), ve(i,j,k+1)
      print*, 'delz: ', dzb(i,j,k), delz(i,j,k), dzb(i+1,j+1,k)
      print*, 'wb: ', wb(i,j,k), w(i,j,k), wb(i+1,j+1,k)
      print*, 'xi: ', xi(i,j,k), xi(i+1,j,k)
      print*, 'eta: ', eta(i,j,k), eta(i,j+1,k)
      print*, 'vortz: ', vortz(i,j,k), vortz(i,j,k+1)
      print*, 'theta: ', theta(i-1:i+1,j,k)
      print*, 'EPV: ', epv(i:i+1,j,k)
   endif
   
!!!END DEBUG CODE!!!
   
  end subroutine ertel_pv
