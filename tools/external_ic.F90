...
!  is Atm%q defined in all cases?
      ...
      if ( Atm(1)%flagstruct%nggps_ic .or. Atm(1)%flagstruct%hrrrv3_ic ) then
        call prt_maxmin('TS', Atm(1)%ts, is, ie, js, je, 0, 1, 1.)
      endif
      if ( Atm(1)%flagstruct%nggps_ic .or. Atm(1)%flagstruct%ecmwf_ic .or. Atm(1)%flagstruct%hrrrv3_ic ) then
      ...
      call p_var(Atm(1)%npz,  is, ie, js, je, Atm(1)%ak(1),  ptop_min,         &
                 ...
                 Atm(1)%flagstruct%hydrostatic, Atm(1)%flagstruct%nwat, Atm(1)%domain, Atm(1)%flagstruct%adiabatic, Atm(1)%flagstruct%make_nh)
...
! convert qncep from NCEP grid to cubic grid
      ...
      call remap_scalar(Atm(1), km, npz, 2, ak0, bk0, psc_r8, qp, gzc)
      call mpp_update_domains(Atm(1)%phis, Atm(1)%domain)
...
!!$      if ( (npz == 64 .or. npz == 63)  .and. len(trim(Atm(n)%flagstruct%npz_type)) == 0 ) then
!!$         if (is_master()) print*, 'Using default GFS levels'
!!$         Atm(1)%ak(:) = ak_sj(:)
!!$         Atm(1)%bk(:) = bk_sj(:)
...
! remap tracers
      ...
      call remap_scalar(Atm(1), km, npz, 6, ak0, bk0, psc_r8, qc, zhc, wc)
      call mpp_update_domains(Atm(1)%phis, Atm(1)%domain)
      if(is_master()) write(*,*) 'done remap_scalar'
...
! Convert excess cloud ice into snow:
 ...
 subroutine remap_dwinds_nh(km, npz, pec, ud, vd, Atm)
  type(fv_atmos_type), intent(inout) :: Atm
  integer, intent(in):: km, npz
  real,    intent(in):: pec(Atm%bd%is-1:Atm%bd%ie+1,Atm%bd%js-1:Atm%bd%je+1, km+1)
...
! local:
  real, dimension(Atm%bd%isd:Atm%bd%ied,Atm%bd%jsd:Atm%bd%jed, km+1):: psd
  ...
  call mpp_update_domains( psd, Atm%domain, complete=.true. )
  call mpp_update_domains( Atm%ps, Atm%domain, complete=.true. )

!$OMP parallel do default(none) shared(is,ie,js,je,npz,km,Atm,psd,ud,vd) &
...
!------
     do k=1,km+1
        do i=is,ie
           pe0(i,k) = 0.5*(psd(i,j-1,k)+psd(i,j,k))
        enddo
     enddo
     do k=1,npz+1
        do i=is,ie
...
5000 continue
 ...
 end subroutine remap_dwinds_nh
